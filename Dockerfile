...
RUN su -c "$VENV/bin/pip install --upgrade 'pip==8.0.2'" $MARV_USER || true
RUN su -c "$VENV/bin/pip install --upgrade 'setuptools==20.0'" $MARV_USER || true
RUN su -c "$VENV/bin/pip install --upgrade 'pip-tools==1.6'" $MARV_USER
